...
function getFirstHeader(markdownContent, defaultTitle) {
  ...
  // 遍历每一行，查找以 # 开头的主标题
  if (firstLine) {
    const match = firstLine.match(/^#\s+(.+)/); // 匹配 # 后面的内容
    if (match) {
      return match[1].trim(); // 返回第一个主标题
    }
  }

  return defaultTitle; // 如果没找到，返回默认标题
}
...
function transformInfoToResult(list, realPath, baseDir) {
  ...
  for (const filePath of listInfo) {
    ...
    const title = getFirstHeader(detail, match?.[1]);
